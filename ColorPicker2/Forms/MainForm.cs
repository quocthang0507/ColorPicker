...
namespace ColorPicker2
{
    public partial class MainForm : Form
    {
        ...
        {
            List<Control> trackBarControls = GetAll(this, typeof(TrackBar)).ToList();
            List<Control> tbxControls = GetAll(this, typeof(TextBox)).ToList();

            foreach (TrackBar trackBar in trackBarControls)
            {
                string tbxName = trackBar.Name.Replace("track", "tbx");
                Control textBox = tbxControls.First(c => c.Name == tbxName);
                if (textBox == null)
                {
		...
		*/
        ...
        {
            IEnumerable<Control> controls = GetAll(groupBox, control.GetType());
